...
fileRouter.route('/')
  ...
  .post(uploader.any(), save_metadata, (req, res, next) => {
    ...
    if (uploaded_file.fieldname !== 'file') {
      res.status(400).json({error: 'No file uploaded'})
    ...
    return FileModel.create({
      ...
      path: res.locals.after_upload.new_filename,
...
fileRouter.get('/:shortcode', async (req, res) => {
  const { shortcode } = req.params
  const { meta } = req.query
  const doc = await FileModel.findOne({shortcode})
  if (!doc) {
